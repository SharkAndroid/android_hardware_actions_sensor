...
static int sensors_detect_devices(const struct sensor_t* slist, int ssize, struct sensor_t* clist, int csize) {
...
sensors_poll_context_t::sensors_poll_context_t() {
    ...
    // detect sensors
    if (sSensorListNum <= 0) {
        sSensorListNum = sensors_detect_devices(sSensorSupportList, ARRAY_SIZE(sSensorSupportList), sSensorList, ARRAY_SIZE(sSensorList));
    }
        ...
        switch (ss->type) {
            case SENSOR_TYPE_ACCELEROMETER:
                if (mSensors[accel] == NULL) {
                    mSensors[accel] = new AccelerationSensor((char*)ss->reserved[0], ss->resolution, ss->minDelay);
...
*/
            ...
            case SENSOR_TYPE_LIGHT:
                if (mSensors[light] == NULL) {
                    mSensors[light] = new LightSensor((char*)ss->reserved[0], ss->resolution, ss->minDelay);
...
*/
            ...
            case SENSOR_TYPE_GYROSCOPE:
                if (mSensors[gyroscope] == NULL) {
                    mSensors[gyroscope] = new GyroSensor((char*)ss->reserved[0], ss->resolution, ss->minDelay);
            ...
            case SENSOR_TYPE_TEMPERATURE:
                if (mSensors[temperature] == NULL) {
                    mSensors[temperature] = new TemperatureSensor((char*)ss->reserved[0], ss->resolution, ss->minDelay);
